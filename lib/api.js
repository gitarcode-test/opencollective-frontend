import { createError, ERROR, getErrorTypeFromHTTPCode } from './errors';
...
import { repeatWithInterval } from './utils';
...
export async function downloadLegalDocument(legalDocument, account, prompt2fa) {
  ...
  const handleResponse = async (response, retryWith2FA) => {
    ...
    if (responseBody.message === 'Two-factor authentication required') {
